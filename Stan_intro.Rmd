...
For referenec, two popular ways to speed up R programs is to rewrite pieces in either `C++`
```{r}
library(Rcpp)
...
system("R CMD SHLIB fibFortran.f90")

dyn.load("fibFortran.so")
func <- getNativeSymbolInfo("fibFortran")
fibFortran <- function(n){
  .Fortran(func, as.integer(n))[[1]]
}
...
```{r}
c(fibR(30), fibStan(30), fibCpp(30), fibFortran(30))
```
...
```
I was surprised to see Stan so much slower than `C++` and `Fortran`. I am curious why this is.
...
gen_kmeans <- stan_model(here("generalized_kmeans.stan"))
...
```{r}
(gen_kmeans_assignemnts == c(rep(1,subset), rep(2, subset), rep(3, subset))) %>% mean
```
...
iris_train <- ln_iris[train,6:9] %>% as.matrix
iris_test  <- ln_iris[test,6:9] %>% as.matrix
